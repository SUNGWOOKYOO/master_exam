...
그래프가 주어졌을 때 어떤 정점들 간에도 간선이 존재하는 정점들의 집합으로 이루어진 부분그래프를 찾는 문제

그래프에서 어떤 pair의 vertices 라도 connected (completely connected) 되어 있는 vertice set 으로 구성된 sub graph를 찾는문제 
...
**reduction  algorithm:** 어떤 한 3-SAT인 boolean equation $\Phi$ 가 주어졌을때, 각각의 literal 에 대해서 vertex를 만들고, 자기가 속한 clause의 literal과 다른 clause에서의 negation을 제외한 나머지 에 대해서 edge를 만든다. ($\Phi$ 안에 총 m 개의 clause가 있다면 m개의 cluster 가 생김). 이 과정은 $O(N^p)$ 걸림.

3- SAT 인 부울식이 주어졌다고 하자.

1. 부울식에 존재하는 모든 literal에 대응하는 정점을 만든다.
2. 한 clause의 literal에 대응하는 정점과 다른 clause에 속한 literal에 대응하는 정점을 전자의 negation을 제외하고 간선을 만든다. 

(여기서 알수있는 것은 생성된 그래프에서 정점이 연결되 있다면, 그에 대응하는 literal 쌍은 동시에 참이다.)

**3-SAT $\Rightarrow $  k-Clique:** 3-SAT의 부울식을 만족한다면 각 clause 마다 적어도 하나의 literal이 참이라는 말이고, 각기 다른 clause 에서 파생된 정점들은 반드시 연결되어 있기 때문에 주어진 clause 크기의 clique는 항상 존재한다. 



**k-Clique $\Rightarrow$ 3-SAT:** k-Clique가 그래프에 있다면, k개의 cluster에서 하나씩 고른 k개의 정점끼리 연결이 되어 있어서 그에 대응되는 clause의 literal이 모두 True가 되므로 3-SAT의 부울식($\Phi$)을 만족할 수 있다.
...
그리고, $E[X]$를 estimate하기위해  i.i.d. $ X_{ij} $ 를 1 if $z_i$  is compared to $z_j$, o.w., 0 으로 정의하고 $P_{ij}$ 를  $P[$ $z_i$  is compared to $z_j]$ 라고 하자, iid 특성은 평균값이 확률값과 같음)  
...
다시 되돌아가서, 계산해보면,
$\sum_{i=1}^{n-1}\sum_{j=i+1}^{n}$$2/(j-i+1)$ $\le n \sum_{j = 1}^{n}{2/j} = O(nlogn)$
...
src, sink node가 있고, nonnegative capacity 를 갖는 간선으로 이루어진 방향그래프 
...
**(1) definition:** 방향그래프에서 모든 정점들을 한번씩만 방문하는 경로가 있는지 찾는 문제

**(2) [HAMPATH is NP](ps://www.geeksforgeeks.org/proof-hamiltonian-path-np-complete/):**  HAMPATH 가 주어지면, 그것은 서로 다른 정점들의 sequence인데, 연속된 정점들을 연결하는 간선들이 그래프에 있는지 확인하는데 polynomial time이 걸린다.
...
​	그래프에서 모든 간선의 양 끝점 중 하나를 포함하는 정점들의 집합들 중 최소의 집합(optimal vertex cover)을 찾는 것
...
​	반복당 C에 추가되는 두 정점을 잇는 간선들의 집합 A 라고하면 $|C| = 2|A|$ 를 만족하고

​	optimal vertex cover를 C*이라고 하자.	

​	A에 있는 어떤 두개의 간선도 양 끝점을 공유하지 않고 

​	C*은 정의에 의해 A의 모든 정점을 cover하기 때문에

​	A에 있는 모든 간선의 양끝점 중 적어도 하나는  C*의 원소이다.   

​	$|C*| \ge |A|$ 가 성립한다.

​	따라서	$|C| = 2|A| \le 2|C*|$  을 만족한다.	 
...
PR(A) = \frac{1-d}{N} + d\{\frac{PR(T_1)}{C(T_1) }+ ... + \frac{PR(T_n)}{C(T_n)}\} = \frac{1-d}{N} + d\sum_{i=1}^{n}\frac{PR(T_i)}{C(T_i)}
$$
