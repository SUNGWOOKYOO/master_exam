...
$$
**SC is NP:** naive 하게 생각해보면 주어진 C'를 가지고 $\underset{s_i \in c'}{\cup}s_i = A$ 이 되는지 확인하면 된다. A에 있는 모든 원소수가 n 개라고 가정하고, 집합 C'의 원소 $s_*$수를 m개 라고하면 $O(nm)$이므로 polynomial time이 걸림
